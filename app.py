        ...
        "messages": [
            {
                ...
                "content": (
                    ...
                    "and visual quality. Return only the improved prompt."
                ),
            },
            {
                "role": "user",
...
def generate_image(prompt, hf_token, max_retries=3):
    ...
    for attempt in range(max_retries):
        ...
        if response.status_code == 503:
            ...
            st.info(
                f"Model is loading. Retrying in {wait_time} seconds..."
            )
        ...
        if response.status_code == 404:
            raise RuntimeError(
                "Hugging Face could not find the model or endpoint.\n\n"
                f"Model: {HF_MODEL_ID}\n"
                f"URL: {HF_API_URL}"
            )
    ...
    raise RuntimeError(
        "The Hugging Face model did not become available "
        "after several attempts."
    )


st.title("🎨 AI Image Generator")
st.write("Create an image from a text prompt using Hugging Face AI.")
st.divider()
...
st.divider()
st.caption("Powered by Hugging Face and Groq")
